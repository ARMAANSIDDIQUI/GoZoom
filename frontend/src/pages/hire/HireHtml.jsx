...
const HireHtml = () => {
    ...
    const strengths = (t('hire_pages.html.strengths', { returnObjects: true }) || []).map((item, i) => ({
        ...item,
        icon: [<FaHtml5 />, <FaCss3Alt />, <FaSearchDollar />][i]
    }));

    const features = (t('hire_pages.html.features', { returnObjects: true }) || []).map((feature, i) => ({
        ...feature,
        icon: [<FaExpandArrowsAlt />, <FaPaintBrush />, <FaPenNib />][i]
    }));
                    ...
                    <div className="flex flex-col lg:flex-row items-center gap-20">
                        <div className="flex-1 text-center" data-aos="fade-right">
                            <div className="inline-flex items-center gap-3 py-2 px-6 bg-orange-500/10 border border-orange-500/20 rounded-full text-orange-400 font-black text-xs mb-8 uppercase tracking-[0.4em]">
                                <FaHtml5 className="text-lg" /> {t('hire_common.pixel_perfect_engineering')}
                            </div>
                            <h1 className="text-5xl lg:text-7xl font-black text-white mb-8 leading-tight">
                                {t('hire_pages.html.heroTitle').split('Responsive')[0]} <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-300">Responsive</span> {t('hire_pages.html.heroTitle').split('Responsive')[1]}
                            </h1>
                            <p className="text-xl text-slate-400 mb-12 max-w-2xl leading-relaxed font-black mx-auto italic">
                                "{t('hire_pages.html.heroDesc')}"
                            </p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-[550px] justify-center">
                                <Link to="/contact" className="inline-flex items-center justify-center w-full sm:w-auto gap-3 px-12 py-5 bg-orange-500 text-white font-black text-center text-lg rounded-2xl shadow-2xl shadow-orange-500/20 hover:scale-105 transition-all group uppercase tracking-wider">
                                    {t('hire_common.enquire_now')} <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
                                </Link>
                                <a href="#design" className="inline-flex items-center justify-center w-full sm:w-auto gap-3 px-12 py-5 border border-white/20 text-white text-center font-black text-lg rounded-2xl hover:bg-white/10 transition-all uppercase tracking-wider backdrop-blur-md">
                        ...
                        <div className="flex-1 relative flex flex-col items-center lg:items-center gap-6" data-aos="zoom-in" data-aos-delay="200">
                            ...
                            <div className="relative w-full max-w-[400px] bg-white border border-slate-100 p-6 rounded-3xl shadow-3xl flex items-center gap-4 group hover:shadow-orange-500/10 transition-all duration-300">
                                ...
                                <div>
                                    <p className="font-black text-lg text-slate-900 uppercase leading-none">{t('hire_common.ui_design_excellence')}</p>
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{t('hire_common.design_to_production')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <section id="design" className="py-32 bg-slate-50">
                <div className="container mx-auto px-6 lg:px-16">
                    <div className="text-center mb-24" data-aos="fade-up">
                        <h2 className="text-4xl lg:text-5xl font-black text-slate-900 mb-6 uppercase tracking-tighter italic">{t('hire_pages.html.fleetTitle').split('Brand')[0]}<span className="text-orange-500">Brand</span> {t('hire_pages.html.fleetTitle').split('Brand')[1]}</h2>
                        <div className="w-24 h-1 bg-orange-500 mx-auto rounded-full mb-8 shadow-lg shadow-orange-500/30"></div>
                        <p className="text-lg text-slate-500 font-bold max-w-2xl mx-auto uppercase tracking-widest">{t('hire_pages.html.fleetDesc')}</p>
                    </div>
                ...
                <div className="container mx-auto px-6 lg:px-16">
                    <div className="grid lg:grid-cols-2 gap-24 items-center">
                        ...
                        <div className="order-1 lg:order-2" data-aos="fade-left">
                            <h2 className="text-4xl lg:text-7xl font-black text-slate-900 mb-12 uppercase leading-none tracking-tighter italic">{t('hire_pages.html.featuresTitle').split('Standard')[0]} <span className="text-orange-500 italic">Standard</span></h2>
                ...
                <div className="container mx-auto px-6 lg:px-16">
                    <div className="relative rounded-[4rem] bg-[#160d01] p-16 lg:p-32 text-center text-white shadow-4xl overflow-hidden" data-aos="zoom-in">
                        ...
                        <div className="relative z-10 max-w-4xl mx-auto">
                            <h2 className="text-5xl lg:text-7xl font-black mb-10 uppercase leading-tight italic tracking-tighter">{t('hire_pages.html.heroTitle').split('Pixel Perfect')[0]} <span className="text-orange-400">Pixel Perfect</span></h2>
                            <p className="text-xl text-slate-400 mb-16 font-bold uppercase tracking-widest">{t('hire_pages.html.heroSubtitle')}</p>
